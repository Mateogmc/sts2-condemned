...
public class SigilOfLooping : CondemnedCard
{
    public SigilOfLooping() : base(0, CardType.Skill, CardRarity.Token, TargetType.None)
    {
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
    {
        ...
        foreach (CardModel card in await CardSelectCmd.FromHand(choiceContext, Owner,
                     new CardSelectorPrefs(SelectionScreenPrompt, amount), c => !c.Keywords.Contains(CondemnedKeywords.Looping), this))
        {
            card.AddKeyword(CondemnedKeywords.Looping);
