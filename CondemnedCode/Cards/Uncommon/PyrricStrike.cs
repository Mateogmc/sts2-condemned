...
public class PyrricStrike : CondemnedCard
{
    public PyrricStrike() : base(1, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
    {
        WithDamage(12, 3);
        ...
        CardPlay play)
    {
        ...
        try {
            ...
            if (IsUpgraded)
            {
                ...
                foreach (CardModel card in Owner.PlayerCombatState.Hand.Cards.ToList().FindAll(c => c.Type != CardType.Curse))
                {
            ...
            {
                CardPile hand = new CardPile(PileType.Play);
                hand.AddInternal(Owner.PlayerCombatState.Hand.Cards.ToList().FindAll(c => c.Type != CardType.Curse).TakeRandom(1, Owner.RunState.Rng.CombatCardSelection).First());
